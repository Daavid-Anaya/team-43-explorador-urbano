...
- [x] 1.1 [A] Crear `package.json`, `vite.config.ts`, `tsconfig*.json`, `src/main.tsx`, `src/app/App.tsx`; AC: la app arranca y existen scripts reales para los gates disponibles; Verificar: `npm install && npm run dev`.
- [x] 1.2 [A] Diseñar y construir la UI de la pantalla principal más el menú de hamburguesa (navbar) con acceso a login; AC: la navegación principal es usable en mobile-first, el ítem de login es visible y accesible sin sesión iniciada; sin lógica de Supabase todavía (eso es 1.2b/1.3); Verificar: `npm run test -- app-shell navbar`.
- [ ] 1.2b [B] Crear `src/lib/supabase/*`, `.env.example`, y `supabase/migrations/*` para perfiles de Auth, desafíos, completados, storage privado de evidencia, y RLS; en paralelo con 1.2, sin dependencia entre ambas; AC: el cliente del navegador usa solo la anon key y el service role está ausente del env del cliente; Verificar: smoke local/proyecto de Supabase cuando esté configurado.
- [ ] 1.3 [A] Vincular los campos de login/signup de la UI de 1.2 al cliente Supabase Auth de 1.2b con email+password, más el contexto de sesión (ej. `src/features/auth/*`); Dependencia: 1.2 (UI) y 1.2b (cliente `src/lib/supabase`) deben existir primero; AC: un visitante puede crear o reanudar una sesión autenticada según `specs/user-identity/spec.md` (el login crea/recupera un perfil e inicia una sesión), un visitante anónimo ve un requerimiento claro de inicio de sesión antes de cualquier acción de persistencia, y no se duplica trabajo de esquema/RLS/migración de Supabase aquí (eso es propiedad de 1.2b); Verificar: `npm run test -- auth login`.
...
- [ ] 1.6 [A+B+C] Parcial: existe `.github/workflows/ci.yml`; falta confirmar que cubra todos los gates reales de build/test/lint/typecheck y documentar gates omitidos. Agregar `.github/workflows/*` de CI una vez que existan scripts npm; AC: el workflow corre gates reales de build/test/lint/typecheck y documenta gates omitidos; Verificar: inspeccionar el workflow más el primer check run del PR.
- [x] 1.7 [A] Agregar `vite-plugin-pwa` (Workbox `generateSW`) con Web App Manifest (name, short_name, iconos 192/512/maskable, theme_color, background_color, `display: standalone`, `start_url`, orientation) y assets de splash iOS/Android; AC: la app califica como instalable según `specs/pwa-shell/spec.md` y el app shell precachea en el build; Verificar: `npm run build` luego auditoría Lighthouse PWA.
