...
- [ ] 1.3 [B] Agregar `supabase/seed/challenges.json` más la ruta de validación; Dependencia: ciudad exacta y dataset semilla aprobados; AC: 8-12 desafíos de una sola ciudad usan los campos/categorías aceptados y existe guía de rollback de semilla; Verificar: `npm run seed:check`. Avance: ciudad decidida (Ciudad de México), `challenges.json` con 10 desafíos, `npm run seed:check` en verde (7/7), guía de rollback en `supabase/seed/README.md`. Pendiente para cerrar: agregar `radiusMeters`, `photoPrompt`, `estimatedMinutes` y el check de categorías a la tabla `public.challenges` (columnas ausentes en la migración actual), y cargar el dataset al proyecto Supabase real.
